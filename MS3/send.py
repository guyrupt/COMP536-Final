...
class KVS(Packet):
    ...
    fields_desc = [
        ...
        BitField("ID", 0, 8),
        # Access info; 0 for action authorized, 1 for write unauthorized, 2 for read & write unauthorized
        BitField("access", 0, 8),
        BitField("ceil", 0, 32),
    ]
...
def main():
    ...
    addr = "10.0.1.1"
    iface = get_if()
    print("Sending request as: (0) Alice (1) Bob")
    user_id = input("Enter user ID: ")
    if not user_id.isdigit() or not 0 <= int(user_id) <= 1:
        print("Invalid user ID")
        exit(1)
    user_id = int(user_id) + 1  # 1 for Alice, 2 for Bob
